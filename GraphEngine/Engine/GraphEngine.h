...
class GraphEngine {
	...
	void create_primary_fbo() {
		...
		float border_ñolor[] = { 0.0, 0.0, 0.0, 0.0 };
	...
	void create_depth_map_fbo() {
		...
		float border_ñolor[] = { 1.0, 1.0, 1.0, 1.0 };
...
public:
	...
	GraphEngine(sf::RenderWindow* window, double fov, double min_distance, double max_distance) {
		...
		cam = eng::Camera(eng::Vect3(0.0, 0.0, 0.0), eng::Vect3(0.0, 0.0, 1.0), fov, min_distance, max_distance, ((double)window->getSize().x) / ((double)window->getSize().y));
	...
	void update(double delta_time) {
		if (active_state) {
			...
			if (sf::Keyboard::isKeyPressed(sf::Keyboard::E))
				cam.rotate(cam.get_direction(), -cam.rotation_speed * delta_time);
