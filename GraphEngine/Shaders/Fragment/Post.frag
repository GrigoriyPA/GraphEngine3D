...
void main() {
    ...
    uint cur = texture(stencil_texture, tex_coord).r;
    ...
    for (int i = -border_width / 2; i <= border_width / 2; i++) {
        ...
        if ((texture(stencil_texture, pos_up).r | cur) > cur || (texture(stencil_texture, pos_down).r | cur) > cur) {
    ...
    for(int i = 0; i < 9; i++)
        frag_color += vec3(texture(screen_texture, tex_coord + offsets[i] * texel_size)) * kernel[i];
