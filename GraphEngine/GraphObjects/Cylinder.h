...
GraphObject get_cylinder(int count_points, bool real_normals = false, int max_count_models = 1) {
	...
	std::vector < Vect3 > positions(count_points);
	for (int i = 0; i < count_points; i++)
		positions[i] = Vect3(cos((2 * PI / count_points) * i), 0, sin((2 * PI / count_points) * i));
	...
	cylinder[polygon_id].change_matrix(trans_matrix(Vect3(0, 1, 0)));
	cylinder[polygon_id].invert_points_order();
	...
	for (int i = 0; i < count_points; i++) {
		...
		positions[i],
		positions[j],
		positions[j] + Vect3(0, 1, 0),
		positions[i] + Vect3(0, 1, 0)
			}, !real_normals);
