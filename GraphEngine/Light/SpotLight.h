...
public:
    ...
    void set_uniforms(int draw_id, eng::Shader<eng::ShaderType::MAIN>* shader_program) {
        ...
        try {
            ...
            shader_program->set_uniform_1i((name + "type").c_str(), 2);
            shader_program->set_uniform_3f((name + "position").c_str(), position.x, position.y, position.z);
            shader_program->set_uniform_3f((name + "direction").c_str(), direction.x, direction.y, direction.z);
            shader_program->set_uniform_1f((name + "cut_in").c_str(), cos(cut_in));
            shader_program->set_uniform_1f((name + "cut_out").c_str(), cos(cut_out));
            shader_program->set_uniform_1f((name + "constant").c_str(), constant);
            shader_program->set_uniform_1f((name + "linear").c_str(), linear);
            shader_program->set_uniform_1f((name + "quadratic").c_str(), quadratic);
