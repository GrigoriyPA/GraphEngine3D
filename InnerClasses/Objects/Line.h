...
class Line : public RenderObject {
    ...
    void update_line(eng::Vec3 point1, eng::Vec3 point2) {
        ...
        (*scene).objects[scene_id.first].models.change_left(scene_id.second, eng::Matrix::translation_matrix((point1 + point2 + direct * length) / 2));
    }
...
public:
    ...
    void switch_hide() {
        ...
        if (!hide) {
            (*scene).objects[scene_id.first].models.change_left(scene_id.second, model * eng::Matrix::scale_matrix(eng::Vec3(1.0 / 3.0, 1.0, 1.0 / 3.0)) * model.inverse());
        } else {
