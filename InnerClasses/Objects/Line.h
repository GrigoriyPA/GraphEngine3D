...
class Line : public RenderObject {
    ...
    void update_line(eng::Vec3 point1, eng::Vec3 point2) {
        if ((point1 - point2).length() < eps)
            point2 += eng::Vec3(1, 0, 0);
        ...
        (*scene)[scene_id.first].models.set(scene_id.second, eng::Matrix::scale_matrix(eng::Vec3(POINT_RADIUS * 0.2, length, POINT_RADIUS * 0.2)));
    ...
    void update_perpendicular_to_line(std::pair < int, int > point, std::pair < int, int > line) {
        ...
        eng::Vec3 proj_point = line_cur.project_point(coord);
        if (proj_point == coord)
    ...
    void update_parallel_to_plans(std::pair < int, int > point, std::pair < int, int > plane1, std::pair < int, int > plane2) {
        ...
        eng::Vec3 direction = (plane_cur1.get_normal() ^ plane_cur2.get_normal()).normalize();
    ...
    void update_point_symmetry(std::pair < int, int > line, std::pair < int, int > center) {
        ...
        eng::Vec3 coord1 = (*scene)[line.first].get_mesh_center(line.second, 0);
        eng::Vec3 coord2 = (*scene)[line.first].get_mesh_center(line.second, 1);
    ...
    void update_bisector_three_points(std::pair < int, int > point1, std::pair < int, int > point2, std::pair < int, int > point3) {
        ...
        eng::Vec3 direction = ((coord1 - coord2).normalize() + (coord3 - coord2).normalize()) / 2;
    ...
    void update_bisector_two_lines(std::pair < int, int > line1, std::pair < int, int > line2) {
        ...
        eng::Vec3 direction2 = line_cur2.get_direction();
        if (direction1 * direction2 * special_coefficient < 0)
            direction2 *= -1;
        eng::Vec3 direction = (direction1 + direction2) / 2;

        eng::Vec3 intersection = line_cur1.intersect(line_cur2);
    ...
    void update_midperpendicular_two_points(std::pair < int, int > point1, std::pair < int, int > point2, std::pair < int, int > plane) {
        ...
        eng::Plane plane_cur(coords);
        eng::Vec3 direction = (line_cur.get_direction() ^ plane_cur.get_normal()).normalize();
    ...
    void update_plan_connect(std::pair < int, int > plane) {
        ...
        eng::Vec3 point1 = plane_proj.project_point((*scene)[scene_id.first].get_mesh_center(scene_id.second, 0));
        eng::Vec3 point2 = plane_proj.project_point((*scene)[scene_id.first].get_mesh_center(scene_id.second, 1));
...
public:
    Line(eng::Vec3 point1, eng::Vec3 point2, eng::GraphEngine* scene) {
        action = -1;
        type = 2;
        this->scene = scene;
    ...
    void switch_hide() {
        ...
        if (!hide) {
            (*scene)[scene_id.first].models.change_left(scene_id.second, model * eng::Matrix::scale_matrix(eng::Vec3(1.0 / 3.0, 1.0, 1.0 / 3.0)) * model.inverse());
        } else {
            (*scene)[scene_id.first].models.change_left(scene_id.second, model * eng::Matrix::scale_matrix(eng::Vec3(3.0, 1.0, 3.0)) * model.inverse());
        }
        hide ^= 1;
    }
    ...
    RenderObject* intersect(RenderObject* obj) {
        ...
        eng::Vec3 coord1 = (*scene)[scene_id.first].get_mesh_center(scene_id.second, 0);
        eng::Vec3 coord2 = (*scene)[scene_id.first].get_mesh_center(scene_id.second, 1);
        eng::Line line_cur(coord1, coord2);
